...
def validar_descricao_produto(descricao: str) -> bool:
    if len(descricao) < 12:
        print("A descricao tem que ter mais de 12 caracteres.")
        return False
    else:
        return True
...
def salvar_dados(produtos, codigo, nome, descricao, categoria, preco, estoque, perecivel):
    
    produtos[codigo] = (nome, descricao, categoria, preco, estoque, perecivel)
    
    
    with open("produtos.txt", "a", encoding="utf-8") as arquivo:
        arquivo.write(f"{codigo};{nome};{descricao};{categoria};{preco:.2f};{estoque};{perecivel}\n")
